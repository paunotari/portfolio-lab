...
| `portfolio/rebalancing.py` | **Rebalancing-scheme check (M22, CLI only)** — the same per-refit weight schedules re-implemented as shipped constant-mix / fully-costed constant-mix / within-interval buy-and-hold (no re-optimization). Verdict: ≤0.002 Sharpe difference, buy-and-hold ranking identical — the constant-mix assumption and the uncosted drift turnover are immaterial. → `optimizer_rebalancing.csv` + `REPORT_rebalancing.md`; deep dive `literature/rebalancing.md`. |

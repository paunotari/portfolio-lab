...
[literature.md](literature.md) — index + verdicts — with implementation-grade deep dives on
two shelves: [literature/classics/](literature/classics/) (the established canon; formulas,
algorithms, pitfalls, codeable without fetching the papers) and
[literature/frontier/](literature/frontier/) (living working papers 2023+, each with a
"⇒ for us" verdict and the candidate test it motivates).
...
| `portfolio/shrinkage.py` | **Covariance estimators** — Ledoit-Wolf constant-correlation target (default, "Honey, I Shrunk…"), scaled-identity variant (sklearn-equivalent test oracle), and **`shrink_nonlinear` (Ledoit-Wolf 2020 ANALYTICAL nonlinear shrinkage** — closed-form Epanechnikov kernel + Hilbert transform, each eigenvalue corrected by its own amount; p ≤ n only, explicit error otherwise). All closed-form, all return `(Sigma, diagnostic)`; δ* is reported in optimizer output as an input-quality number. **`estimate_covariance(returns, method)` is the dispatcher** every risk number goes through, keyed on `config.OPTIMIZER_SIGMA_ESTIMATOR` — so the estimator is a sensitivity-grid dimension rather than a hard-coded assumption. |
| `portfolio/anchors.py` | **μ-free structural engines**: `equal_weight` (1/N), `erc_weights` (equal risk contribution, Spinu convex form — the optimizer's neutral anchor; **`budgets=` parameter (2026-07) generalizes to per-sleeve risk budgeting** — asset i contributes share b_i of portfolio risk, equal budgets = classic ERC), `hrp_weights` (López de Prado's 3-stage HRP), **`herc_weights` (Raffinot 2018 HERC — ERC on HRP's topology: Ward or single linkage, `gap_index` cluster count, exact ERC solves both BETWEEN sibling clusters (on their true 2×2 covariance) and INSIDE terminal clusters)**, `min_var_weights`, `risk_contributions` (Euler RC vector, reused in reporting). All long-only, sum to 1, consume the shrunk Σ. |
...
| `portfolio/rules.py` | **Rule-based strategies tested as walk-forward contestants** (not optimizer portfolios): `momentum_weights` (Jegadeesh-Titman cross-sectional 12-1, top-K equal weight) and `vol_managed` (unlevered Moreira-Muir volatility targeting — scale a base portfolio's exposure down toward a vol target, hold cash, never lever up; causal) and **`gmv_combo_weights`** (Yuan-Zhou 2023 GMV combination — the humility claim's strongest published challenger: λ*·GMV + (1−λ*)·1/N with λ* re-derived in the docstring from their five stated scalars; the table's only UNCONSTRAINED contestant, deliberately). **`brodie_weights`** (Brodie et al. 2009 sparse-and-stable Markowitz in its long-only form, where the L1 penalty is provably inert = min-variance at the trailing-1/N target return) and **`trend_overlay`** (Faber 2007's 10-month SMA switch and the absolute-momentum leg of Antonacci's dual momentum — binary, unlevered, causal, own turnover charged). `OPTIMIZER_MOMENTUM_*` / `OPTIMIZER_VOLTARGET_*` / `OPTIMIZER_TREND_*` config. 2026-07 verdicts: momentum and vol-target beat neither min-variance nor 1/N; the **GMV combo LOSES to 1/N (0.735 vs 0.830, p 0.449) exactly as their own theory predicted — M26**; **Brodie is 3rd at 0.933 but p 0.149 vs 1/N, so their "significantly" does not replicate — M28**; **Faber buys drawdown not Sharpe (maxDD −19.9% vs −26.9%, Sharpe 0.656) and dual momentum is significantly WORSE than 1/N (p 0.034) — M30**. |
| `portfolio/validation.py` | **Walk-forward OOS backtest** (literature directive 7): expanding window (120m warmup, annual refits, `OPTIMIZER_WF_*` config), everything re-estimated on train only; contestants 1/N, min-var, ERC, HRP, balanced sliders, maximin (±geo-cap), **plus the `rules.py` challengers (momentum, vol-target overlays, the Yuan-Zhou GMV combo)**; returns **net of `OPTIMIZER_TC_BPS` transaction cost** on one-way turnover (`oos_sharpe_gross` = pre-cost) → `optimizer_walkforward.csv`/`_returns.csv` + report section. First result (2026-07): min-var best OOS Sharpe (net); balanced blend, momentum and vol-targeting all did NOT beat it — stated in the report, per the honesty principle. Also fields the **all-weather diversified maximin on its own extended universe** — verdict (MILESTONES M7): OOS Sharpe 0.94, best after era-flagged min-var, lowest vol in the table (10.6%), maxDD −22.5%. **Exposure robustness (M12)**: `exposure_diagnostics()` (half-sample Sharpe split, rolling-36m beats-1/N share, correlation with each region Reference → `optimizer_exposure.csv` + a standing report subsection) runs on every build; `--loro [REGIONS]` CLI re-runs the whole walk-forward dropping one region at a time (expensive, like A2's `--dispersion`) → `optimizer_loro.csv` + `REPORT_exposure_robustness.md`. First read: min-var/ERC/HRP beat 1/N in 98-100% of rolling windows; equity maximin only 33%. **M21**: per-refit weights stored in walk-forward meta (`_weights`, plus `_gross`/`_turnover` for the cost grid); `sleeve_attribution()` → `optimizer_attribution.csv` + report section (min-variance = 82% Quality; all-weather gold = 33%); `inference.pbo_cscv` PBO 33% standing in the report. |
| `portfolio/placebo.py` | **Random-regime placebo (M32, CLI only)** — the permutation test behind the regime layer: the walk-forward stripped to the label-consuming contestants (maximin family + 1/N as an invariance control), re-run B times with the macro-state labels SCRAMBLED. Two nulls: `circular` (rotate the label frame — marginals, run lengths and the transition matrix preserved EXACTLY, only the alignment with returns destroyed; the primary) and `iid` (rows permuted — persistence destroyed too). **Two metrics**, because Sharpe alone would test the maximin on a target it never claimed: net OOS Sharpe AND the realized worst-**REAL**-quadrant floor for whatever weights each arm chose (scoring a scrambled-label portfolio against its own scrambled quadrants would be circular). Empirical one-sided permutation p → `optimizer_placebo.csv` + `REPORT_placebo.md`. `optimizer.build_inputs(states=...)` exists solely to feed it. Conservative by construction (the rotation can hand a training window labels from after its own end, which can only help the placebo). **VERDICT (M32): the labels do NOT survive it** — nothing significant in 12 cells (best p 0.195), real below the scrambled mean in 7 of 12. Every number in M6/M7/M10 stands; the ATTRIBUTION does not. Wherever the flagship is described, the phrase is "a capped worst-case objective over a regime-DIVERSE MENU", never "a regime-aware portfolio". |
| `portfolio/rebalancing.py` | **Rebalancing-scheme check (M22, CLI only)** — the same per-refit weight schedules re-implemented as shipped constant-mix / fully-costed constant-mix / within-interval buy-and-hold (no re-optimization). Verdict: ≤0.002 Sharpe difference, buy-and-hold ranking identical — the constant-mix assumption and the uncosted drift turnover are immaterial. → `optimizer_rebalancing.csv` + `REPORT_rebalancing.md`; deep dive `literature/classics/rebalancing.md`. |
| `portfolio/sensitivity.py` | **Sensitivity grids (M17, CLI only)** — costs 0/10/25 bps (re-netted from the walk-forward's cached gross+turnover, no re-optimization), refits 6/12/24m, diversified-cap levels, LW bootstrap block 3/6/10, **covariance ESTIMATOR (constant-correlation / identity / LW-2020 nonlinear)**; tracks whether any LEDGER conclusion (C1 min-var #1 · C2 nothing-beats-1/N · C3 capping helps · C4 all-weather podium) flips per cell → `optimizer_sensitivity.csv` + `REPORT_sensitivity.md`. 2026-07 verdicts: plateau everywhere except the estimator column — **M33: nonlinear shrinkage costs min-variance 0.084 Sharpe (its edge is partly the crude estimator's doing), though C1 still does not flip**; C2 is a frontier (p crosses 5% at block=3) — reported as borderline. The verdict section now separates **stale conclusions** (false in the shipped cell — e.g. M17's C4 after the field grew 12→17) from **sensitivity flips**, so a grid never takes the blame for something the baseline did on its own. |
...
| `portfolio/inference.py` | **Sharpe-ratio inference (paper-track gate 1, 2026-07)** — Ledoit-Wolf (2008) test for Sharpe differences (delta-method HAC z + primary **studentized circular block bootstrap** p-value; `OPTIMIZER_INFER_B`/`_BLOCK` config) and **deflated Sharpe** (Bailey-LdP 2014, multiplicity-adjusted P(true SR>0)). `inference_table()` runs every contestant vs 1/N and vs Min-variance on the walk-forward net OOS returns → `optimizer_inference.csv` + a standing report section with auto-verdict. Method deep dive: `info/literature/classics/sharpe-inference.md`. First verdict (M14): nothing beats 1/N at 5% (min-var p_boot 0.055); two overlays significantly worse. **Added 2026-08 after the paper's own peer-review pass exposed two gaps (M38/M39): `sharpe_power()`** — power and MDES for a Sharpe-difference test, computed on the SAME HAC s.e. as the test, so a non-rejection can be reported as "no effect *resolvable at this sample size*" instead of "no effect" (headline verdict: **56% power**, MDES +0.27 vs an observed +0.20 — the modern race is unresolved, not settled); and **`dr2_bootstrap()`** — circular block bootstrap for the Choueifaty-Coignard DR², resampling months and **re-shrinking the covariance inside each replicate**, giving a percentile CI for one menu and a PAIRED test of two (equity 1.310 [1.242, 1.399]; equity→extended +0.120 at p=0.0002). DR² had been the paper's contribution while carrying no inference at all; it now clears a bar the Sharpe race does not. |

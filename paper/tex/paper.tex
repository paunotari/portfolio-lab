...
has \textbf{56\% power} at the 5\% level. The smallest gap the design could detect at a given target power follows from the same standard
error. Writing $\pi$ for that target and $\alpha$ for the significance level,
...
bracket is 2.80, so the minimum detectable gap is 2.80 standard errors, or \textbf{$+0.27$}
...
general; every portfolio here is long-only, so $w \ge 0$.) That number is \textbf{1.31} for the

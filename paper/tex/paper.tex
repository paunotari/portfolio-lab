...
\newcommand{\exhibittitle}[2]{%
  ...
  \par\noindent\textbf{Exhibit \theexhibit: #1}\label{#2}%
  \par\vspace{5pt}}
\newcommand{\exhibitnote}[1]{%
  \par\vspace{5pt}\noindent{\fontsize{8}{9.6}\selectfont #1}\par}
...
inference (the robust successor to the \citeauthor{jobson1981} test,
\citeyear{jobson1981}, that the prior replication uses), deflated Sharpe ratios \citep{bailey2014}, a Friedman-Nemenyi joint test
...
returns, common window January 1999 to June 2026 (330 months). The menu is redundant by
...
ranking flip. Rightmost panel: the study's one genuine frontier, the headline $p$-value's
...
\exhibitnote{Panel A: the most correlated cut of the menu is same-region, different-factors
(0.885), and only the non-equity sleeves are genuinely different bets (Treasuries $-0.115$,
gold $-0.007$). Panel B: independent risk bets ($DR^{2}$). The shaded band below 1.0 is the
...
equal-weighted equity menu, with a 95\% confidence interval of \textbf{1.24 to 1.40}. The menu
...
\textbf{1.28} independent bets, so the engine whose entire job is to exploit correlations to
cancel risk, holding a quarter as many sleeves, finds no more of them than naive diversification
does. And twenty-four extra equity sleeves buy 0.03 of a bet. The menu is one dominant factor
wearing 28 names, which the eigenvalues confirm: the first principal component alone explains
...
\exhibitnote{The permutation test against our own signature feature. Grey: net out-of-sample

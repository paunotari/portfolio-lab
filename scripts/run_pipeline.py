...
Steps:
  ...
  10. portfolio.optimizer  anchor + benchmarks + flagship portfolios + walk-forward -> outputs/analytics/optimizer
  11. dashboard.build     self-contained outputs/dashboard.html
"""
import sys
import argparse
...
def main():
    ...
    print("=== 1/11 ingest returns ===");     returns.run()
    print("=== 2/11 ingest factsheets ===");  factsheets.run()
    print("=== 3/11 ingest asia images ==="); asia_images.run()
    if not args.no_macro:
        print("=== 4/11 ingest macro (FRED) ==="); macro.run()
    print("=== 5/11 analytics ===");          engine.run()
    if not args.no_macro:
        print("=== 6/11 macro-link ===");      macro_link.run()
        print("=== 7/11 macro-state ===");     macro_state.run()
        print("=== 8/11 scenario ===");        scenario.run()
    print("=== 9/11 diversification ===");    diversification.run()
    print("=== 10/11 optimizer ===");         optimizer.run()
    if not args.no_dashboard:
        print("=== 11/11 dashboard ===");     dashboard_build.run()
    print("\nPipeline complete.")
